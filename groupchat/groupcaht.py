...
sv = Service('群管', enable_on_default=True, help_ = sv_help)
...
async def set_group_card(bot, event):
    ...
    role = owner['role']
    if role == 'member':
        await bot.send(event, '你喉辣么大声赣神魔嘛，咱又不是管理，要改群名片去找管理啊()')
        return
    if not msg:
    ...
    if u_priv >= sv.manage_priv:
        ...
        await bot.send(event, '只有管理以上才能修改他人群名片，咱不能造反')
...
async def kuangwo(bot, event):
    ...
    txt = requests.get(url=url, timeout=10)
    txt1 = txt.json()
    msg = txt1['data']['text']
    # msg = MessageSegment(type_='text', data={'text': txt1})
